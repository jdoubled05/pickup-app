...
function CourtsMapComponent({
  ...
  const supercluster = useMemo(() => {
    ...
    const features: CourtFeature[] = courtsWithCoords.map((court) => ({
      type: "Feature",
      geometry: { type: "Point", coordinates: [court.longitude!, court.latitude!] },
      properties: {
        courtId: court.id,
  ...
  const handleMarkerPress = useCallback(
    (courtId: string) => { onSelectCourt?.(courtId); },
    [onSelectCourt]
  );
  ...
  const handleClusterPress = useCallback(
    (clusterId: number, coordinate: { latitude: number; longitude: number }) => {
      ...
      mapRef.current?.animateToRegion(
        { ...coordinate, latitudeDelta: longitudeDelta, longitudeDelta },
        400
      );
    },
    [supercluster]
        ...
        if ("cluster" in cluster.properties && cluster.properties.cluster) {
          ...
          return (
            ...
            >
              ...
              >
                {hasHot && <Text style={{ fontSize: 9, lineHeight: 10 }}>🔥</Text>}
                <Text
                  style={{
